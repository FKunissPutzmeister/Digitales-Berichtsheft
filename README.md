...
- **Session-Store:** `session-file-store` wirft unter Windows sporadisch `EPERM` beim atomaren Rename. Der Schreibpfad (`set`/`touch`) hat KEINE Retries (die greifen nur beim `get`), daher wurde der per-Request TTL-Bump (`touch`) als `[unhandled]` in den Fehlerbericht gespammt. Behoben via `hardenWrites` (`services/session-store.js`): Schreib-Retries für `set`/`touch`; ein endgültiger `touch`-Fehler (reiner TTL-Bump) wird geschluckt, `set`-Fehler bleiben sichtbar. Für Produktion robusteren (DB-gestützten) Store erwägen.
